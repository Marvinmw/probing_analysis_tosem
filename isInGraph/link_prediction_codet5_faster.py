...
class CodeLinkDatasetFromFile(Dataset):
    ...
    def __getitem__(self, item):
        ...
        #print(self.examples[item].label_id)
        #logger.info(self.examples[item].label_id)
        return (
            torch.tensor(
                        self.examples[item].ex1.spanarray
            ),
            torch.tensor(
                        self.examples[item].ex1.spanarray_mask
            ),
            torch.tensor(
            ...
            torch.tensor(
                        self.examples[item].ex2.spanarray_mask
    ...
    def check_input(self):
        # remove index more than 512
        new_examples = []
        for item in self.examples:
            ex1 = item.ex1.spanarray
            ex2 = item.ex2.spanarray
            index = item.ex1.index
            h= code_features[index][self.args.layer].squeeze(0)
            # print(len(h))
            # print(ex1)
            ex1 = [d for sublist in ex1 for d in sublist]
            ex2 = [d for sublist in ex2 for d in sublist]
            #print( max(ex1))
            # assert max(ex1) < len(h), f"{max(ex1)} {len(h)}"
            # assert max(ex2) < len(h)
            if max(ex1) < len(h) and max(ex2) < len(h):
...
def train(args, train_dataset, eval_dataset, model):
    ...
    for idx in range(args.epochs):
        ...
        for step, batch in enumerate(bar):
            (ex1_span, ex1_mask, ex2_span, ex2_mask,label, input_features) = [
                x.to(args.device) for x in batch
            ]
            model.train()
            loss, logits = model(input_features, ex1_span, ex1_mask, ex2_span, ex2_mask,label)
...
def evaluate(args, model, eval_dataset, eval_when_training=False):
    ...
    for step, batch in enumerate(bar):
        (ex1_span, ex1_mask, ex2_span, ex2_mask, labels, input_features) = [
            x.to(args.device) for x in batch
        ]
        with torch.no_grad():
            lm_loss, logit = model(input_features, ex1_span, ex1_mask, ex2_span, ex2_mask, labels)
...
if __name__ == "__main__":
    ...
    if args.dataset == "poj-104":
        ...
        res = {}
        programs_ids = json.load( open("../datasets/poj-104/isINGraphs/exp1/program_ids.json", "r") )
        with open(normalized_source_code_file) as f:
            ...
            for l in lines:
                ...
                index = l_code["index"]
                code = l_code["code"]
                if index in programs_ids:
                    res[index] = code
        json.dump(res, open("../datasets/poj-104//poj-104/test/sample_1000_formated_source_code_clang.json", "w"))
        normalized_source_code_file = "../datasets/poj-104/poj-104/test/sample_1000_formated_source_code_clang.json"
    ...
    # Setup logging
    logger = Log.get_logger(name="isIngraph", logs_dir=args.output_dir)
    ...
    hidden_folder_file = args.data_folder.replace("isINGraphs", "graphs")
    ...
    dataset_train.check_input()
    dataset_valid.check_input()
    dataset_test.check_input()
